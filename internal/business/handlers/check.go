...
func (ch checkHandler) readiness(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	ctx, span := trace.SpanFromContext(ctx).Tracer().Start(ctx, "handlers.check.readiness")
	defer span.End()

	status := "ok"
	statusCode := http.StatusOK
	if err := database.StatusCheck(ctx, ch.db); err != nil {
		status = "db not ready"
...
func (ch checkHandler) liveness(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
